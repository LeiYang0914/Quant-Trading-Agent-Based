...
- 2026-06-14 (session 12): **Security: hardcoded Glassnode API key removed.** Found Glassnode API key `32YB16...` hardcoded as fallback in `src/data/glassnode_loader.py` and `src/data/glassnode_btc_etf_flows.py`. Key had been pushed to GitHub in commits `2c63f1d` and `f51941e`. Remediation: (1) removed hardcoded fallback from both files — now `os.getenv("GLASSNODE_API_KEY", "")` only, with clear ValueError when unset; (2) added key to gitignored `.env` file; (3) set persistent Windows user-level env var `GLASSNODE_API_KEY`; (4) scrubbed entire git history via `git filter-branch --tree-filter` replacing key with `REDACTED_ROTATE_THIS_KEY`; (5) force-pushed cleaned history to GitHub. **Action required:** Rotate the Glassnode API key at glassnode.com — old key was publicly exposed on GitHub before the scrub.
